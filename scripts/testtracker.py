...
data = numpy.genfromtxt("data/track_data_1717484844.csv", delimiter=",", skip_header=1)
sample_rate = 400  # 100 Hz

timestamp = data[:, 9]
gyroscope = data[:, 3:6]
accelerometer = data[:, 0:3]
magnetometer = data[:, 6:9]  # Assuming magnetometer data is in columns 6-8

# Plot sensor data
figure, axes = pyplot.subplots(nrows=7, sharex=True, gridspec_kw={"height_ratios": [6, 6, 6, 6, 2, 1, 1]})
...
ahrs.settings = imufusion.Settings(imufusion.CONVENTION_NWU,
                                   ...
                                   20,  # magnetic rejection (increase for higher reliance on magnetometer)
...
axes[4].plot(timestamp, internal_states[:, 0], "tab:olive", label="Acceleration error")
axes[4].set_ylabel("Degrees")
axes[4].grid()
axes[4].legend()

axes[5].plot(timestamp, internal_states[:, 1], "tab:cyan", label="Accelerometer ignored")
pyplot.sca(axes[5])
pyplot.yticks([0, 1], ["False", "True"])
axes[5].grid()
axes[5].legend()

axes[6].plot(timestamp, internal_states[:, 2], "tab:orange", label="Acceleration recovery trigger")
